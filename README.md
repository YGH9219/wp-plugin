...
- 대표이미지 또는 수동 AI 배경을 소셜·Google 검색용 4종 브랜드 JPG로 자동 생성
...
- WordPress 6.3 이상
- PHP 7.4 이상
- FreeType 포함 GD 또는 Imagick 이미지 확장
- OpenAI API 키
...
`설정 → 브랜드 썸네일`에서 기능·로고·브랜드 색상을 정합니다. 별도 로고를 고르지 않으면 현재 테마의 사이트 로고를 사용합니다. 글 편집 화면의 `브랜드 썸네일` 패널에서는 최대 두 줄 문구와 이미지 초점을 정합니다. 첫 줄은 흰색, 둘째 줄은 브랜드 색상으로 합성되며 카테고리·흰 여백·테두리는 넣지 않습니다.

대표이미지를 저장하면 원본은 건드리지 않고 업로드 폴더의 `personal-cta-social` 디렉터리에 1200×630, 1200×675, 1200×900, 1200×1200 JPG를 새로 만듭니다. `AI 배경 생성`은 설정된 OpenAI 키로 글 내용에 맞는 글자 없는 배경을 한 번 생성한 뒤 같은 템플릿을 적용합니다. 저장할 때 자동으로 OpenAI를 호출하지 않으므로 버튼을 누른 경우에만 이미지 API 비용이 발생합니다.
