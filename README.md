...
- 원문 사실 분석 → 독립 초안 3개 → 편집장 재작성 → 전환력 품질 심사
- 설명형 결과만 한 번 더 전환형 문구로 보정(반복 재시도 없음)
- 원문 근거 기반 긴장감 → 즉시 이유·행동 → 핵심 → CTA로 재구성
...
6. 생성 중에는 현재 단계와 마지막 서버 활동을 확인합니다. 전환력 심사에서 설명형 결과로 판정되면 한 번만 보정 단계가 추가됩니다.
7. 결과가 기대와 다르면 패널 아래의 `생성 단계 진단 (관리자 전용)`을 열어 Writer 초안 3개, 편집장 결과, 최종 보정 결과를 비교합니다.
8. 준비된 문구를 `복사`해 Threads에 직접 붙여넣고 업로드합니다.

상단 아이콘, 패널과 REST 요청은 해당 글을 편집할 수 있는 `manage_options` 권한 사용자에게만 제공됩니다.

진단 화면은 원문, FACT 근거, API 응답 ID와 키를 표시하지 않습니다. 생성 중에는 저장된 단계만 2.5초 간격으로 다시 조회합니다.
...
3. 같은 버전의 태그를 푸시합니다. 예: `git tag v0.3.4` 후 `git push origin v0.3.4`.
